...
static jl_value_t *jl_new_bits_internal(jl_value_t *dt, void *data, size_t *len)
{
    ...
    *len += nb;
    ...
    jl_value_t *v = jl_gc_alloc_ty(ptls, nb, bt);
    switch (nb) {
...
JL_DLLEXPORT jl_lambda_info_t *jl_new_lambda_info_uninit(void)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_lambda_info_t *li =
        (jl_lambda_info_t*)jl_gc_alloc_ty(ptls, sizeof(jl_lambda_info_t),
                                          jl_lambda_info_type);
    li->code = NULL;
...
JL_DLLEXPORT jl_method_t *jl_new_method_uninit(void)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_method_t *m =
        (jl_method_t*)jl_gc_alloc_ty(ptls, sizeof(jl_method_t), jl_method_type);
...
JL_DLLEXPORT jl_methtable_t *jl_new_method_table(jl_sym_t *name, jl_module_t *module)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_methtable_t *mt =
        (jl_methtable_t*)jl_gc_alloc_ty(ptls, sizeof(jl_methtable_t),
                                        jl_methtable_type);
...
JL_DLLEXPORT jl_typename_t *jl_new_typename_in(jl_sym_t *name, jl_module_t *module)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_typename_t *tn =
        (jl_typename_t*)jl_gc_alloc_ty(ptls, sizeof(jl_typename_t),
                                       jl_typename_type);
    tn->name = name;
    tn->module = module;
    tn->primary = NULL;
...
JL_DLLEXPORT jl_datatype_t *jl_new_uninitialized_datatype(size_t nfields, int8_t fielddesc_type)
{
    ...
    jl_datatype_t *t =
        (jl_datatype_t*)jl_gc_alloc_ty(ptls, (sizeof(jl_datatype_t) +
...
#endif
    jl_typector_t *tc =
        (jl_typector_t*)jl_gc_alloc_ty(ptls, sizeof(jl_typector_t),
                                       jl_typector_type);
    tc->parameters = p;
    tc->body = body;
...
#define BOXN_FUNC(nb,nw)                                                \
    ...
    {                                                                   \
        ...
        jl_value_t *v = jl_gc_alloc_ty(ptls, nw * sizeof(void*), t);    \
...
#define BOX_FUNC(typ,c_type,pfx,nw)                                     \
    ...
    {                                                                   \
        jl_ptls_t ptls = jl_get_ptls_states();                          \
        jl_value_t *v = jl_gc_alloc_ty(ptls, nw * sizeof(void*),        \
                                       jl_##typ##_type);                \
...
jl_expr_t *jl_exprn(jl_sym_t *head, size_t n)
{
    ...
    JL_GC_PUSH1(&ar);
    jl_expr_t *ex = (jl_expr_t*)jl_gc_alloc_ty(ptls, sizeof(jl_expr_t),
                                               jl_expr_type);
    ex->head = head;
    ex->args = ar;
