...
static jl_value_t *jl_type_union_v(jl_value_t **ts, size_t n)
{
    ...
    JL_GC_PUSH1(&types);
    jl_uniontype_t *tu =
        (jl_uniontype_t*)jl_gc_alloc_ty(ptls, sizeof(jl_uniontype_t),
                                        jl_uniontype_type);
    tu->types = types;
    jl_gc_wb(tu, types);
    JL_GC_POP();
                                 ...
                                 jl_value_t **env, size_t n)
{
    ...
    if (!istuple && ndt->name->names == jl_emptysvec) {
        ...
        if (jl_is_datatype_make_singleton(ndt)) {
            ndt->instance = jl_gc_alloc_ty(ptls, 0, ndt);
...
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_tvar_t *tv = (jl_tvar_t*)jl_gc_alloc_ty(ptls, sizeof(jl_tvar_t),
                                               jl_tvar_type);
    tv->name = name;
    tv->lb = lb;
    tv->ub = ub;
...
void jl_init_types(void)
{
    ...
    jl_nothing = jl_gc_alloc(ptls, 0);

    jl_emptysvec = (jl_svec_t*)jl_gc_alloc_ty(ptls, sizeof(void*),
    ...
    jl_emptytuple = jl_gc_alloc_ty(ptls, 0, empty_tuple_type);
