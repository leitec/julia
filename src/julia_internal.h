...
STATIC_INLINE jl_value_t *jl_gc_alloc_(jl_ptls_t ptls, size_t sz)
{
    ...
    if (allocsz <= GC_MAX_SZCLASS + sizeof(jl_taggedvalue_t)) {
        ...
        return jl_gc_pool_alloc(ptls, p, osize, endoff);
    }
    return jl_gc_big_alloc(ptls, allocsz);
}
JL_DLLEXPORT jl_value_t *jl_gc_alloc(jl_ptls_t ptls, size_t sz);
// On GCC, only inline when sz is constant
#ifdef __GNUC__
#  define jl_gc_alloc(ptls, sz)                                         \
    (__builtin_constant_p(sz) ? jl_gc_alloc_(ptls, sz) : jl_gc_alloc(ptls, sz))
#else
#  define jl_gc_alloc(ptls, sz) jl_gc_alloc_(ptls, sz)
#endif

STATIC_INLINE jl_value_t *jl_gc_alloc_ty(jl_ptls_t ptls, size_t sz,
                                         jl_datatype_t *ty)
{
    jl_value_t *v = jl_gc_alloc(ptls, sz);
    jl_set_typeof(v, ty);
    return v;
}
...
{
    jl_value_t *v = jl_gc_alloc(ptls, sz);
    jl_astaggedvalue(v)->header = jl_buff_tag;
    return v;
}

STATIC_INLINE jl_value_t *newstruct(jl_ptls_t ptls, jl_datatype_t *type)
{
    return jl_gc_alloc_ty(ptls, type->size, type);
}
